...
def get_station_map(root_path, stations):
    # filename = '../04.지도시각화/keys/도로명주소apikey.txt'        # 08.map.py가 작동하는 위치를 기준 
    ...
    filename = os.path.join(root_path, 'static/img/station_map.html')   # return map도 가능. 그냥 약속한 root
    map.save(filename)
...
def get_cctv(static_path):
    ...
    folium.Choropleth(
        geo_data=geo_data,
        data=df.CCTV댓수,           # df.column X df[column] O
...
def get_cctv_pop(static_path, column, colormap):
    ...
    folium.Choropleth(
        geo_data=geo_data,
        data=df[column],           # df.column X df[column] O
        columns=[df.index, df[column]],
        fill_color=colormap,
        key_on='feature.id'         # get_data에 있는 feature.id와 join
    ).add_to(map)
